...
//Number of tries allowed to find the word (take out due to autosolver using multiple words)
//#define MAX_TRIES 6 (no longer needed so commented out)
...
void help() {
	printf("Wordle is a simple game:" EOL "Guess the %i letter word within unlimited tries" EOL, WORD_LENGTH); // MAX_TRIES here *** within %i tries...
	puts(
...
void gameLoop() {
	...
	puts(
		...
		"====\t===============");
	FILE * fp;
    fp = fopen("lists/ALL.TXT", "r");

//started editing code from here

	while (strcmp(guess, word)) {




		printf("Guess %i: ", guesses++);
    	if ((((scan = fscanf(fp, "%[^\n]", guess)) != EOF)) == 1 && strlen(guess) == WORD_LENGTH) { //reads each line of the text file and uses the word as an output
			printf("%s\t", guess); // shows the word that is guessed
			toLower(guess);
			...
			printf("Invalid word. Must be %i characters\n", WORD_LENGTH);  //check for invalid words
		}

	if (strcmp(guess, word)) {
		fgetc(fp);                     //fgetc function to check if word is right or Not
	} else {
		printf("\nThe word was %s\t", word); // you guessed correctly
		puts("You win");
		}
	}
}

//rest of the code is from the professors github
